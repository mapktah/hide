...
PROGRAM_NAME="Hide Server"
SCRIPT_DIR="shellscripts"
PYTHON_VER="3.6"
USE_GUNICORN=0
GUNICORN_WORKERS=2
GUNICORN_WORKER_TYPE="gthread"
GUNICORN_WORKER_TYPE_FLAG="--thread=4"
SOURCE_DIR="../src"
COMPILE_MODULE="."
MODULE_TO_RUN="hide.RestApi"
...
if [ $USE_GUNICORN -eq 0 ]; then
  ...
  echo "[$SCRIPT_NAME] Starting $PROGRAM_NAME, $GUNICORN_WORKERS gunicorn workers, type '$GUNICORN_WORKER_TYPE', worker flags '$GUNICORN_WORKER_TYPE_FLAG'.."
  PYTHONPATH="$PROJECTDIR"/"$SOURCE_DIR":"$EXTERNAL_SRC_FOLDERS" \
   $GUNICORN_BIN \
